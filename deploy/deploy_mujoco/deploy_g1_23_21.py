...
if __name__ == "__main__":
    ...
    policy_path = "/home/mycode/ASAP/logs/MotionTracking/20250418_191802-MT-motion_tracking-g1_23to21/exported/model_27200.onnx"
    ...
    xml_path = "/home/mycode/ASAP/humanoidverse/data/robots/g1/g1_23to21.xml"
    ...
    dof_pos_buf = np.zeros(21 * history_length, dtype=np.float32)
    dof_vel_buf = np.zeros(21 * history_length, dtype=np.float32)
    action_buf = np.zeros(21 * history_length, dtype=np.float32)
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        while viewer.is_running() and time.time() - start < simulation_duration:
            ...
            if counter % control_decimation == 0:
                ...
                dof_pos_buf = np.concatenate((dof_pos, dof_pos_buf[:-21] ), axis=-1, dtype=np.float32)
                dof_vel_buf = np.concatenate((dof_vel, dof_vel_buf[:-21] ), axis=-1, dtype=np.float32)
                action_buf = np.concatenate((action, action_buf[:-21] ), axis=-1, dtype=np.float32)
